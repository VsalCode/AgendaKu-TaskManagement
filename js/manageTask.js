document.addEventListener("DOMContentLoaded", () => {
  ...
  // Kapital huruf pertama task
  function capitalizeFirstChar(str) {
    if (!str) return str;
    return str.charAt(0).toUpperCase() + str.slice(1);
  }

  // Format Tanggal agar bulan string
  ...
  // Membuat instance dari class Task
  const myTasks = new Task();

  // Membuat variabel untuk mengambil seluruh task
  const existingTask = myTasks.getTask();

  // Tempat kondisi task
  const taskWrapper = document.getElementById("taskWrapper");
  const taskWrapperEmpty = document.getElementById("taskWrapperEmpty");

  function displayTask(tasks = existingTask) {
    // Task Kosong
    if (tasks.length === 0) {
    ...
    // Ada task
      ...
      // forEach untuk mengulang tampilan HTML
      tasks.forEach((task) => {
        // deklarasi format date
        const userFriendlyDate = formatDate(task.taskCreated);
            ...
            ${
              ...
              <div class="flex shrink-0 w-5 h-5">
                <svg width="20" height="21" viewBox="0 0 20 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                  ...
                  <path d="M4.29163 3.83337H13.625C15.875 3.83337 16.375 5.08337 14.7916 6.66671L13.7916 7.66671C13.125 8.33337 13.125 9.41671 13.7916 10L14.7916 11C16.375 12.5834 15.7916 13.8334 13.625 13.8334H4.29163"
              ...
              <p>In Progress</p>
                ...
                : `<div class="flex gap-1 items-center text-taskia-green bold">
              <div class="flex shrink-0 w-5 h-5">
                <svg width="20" height="21" viewBox="0 0 20 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M4.29163 2.16663V18.8333" stroke="currentColor" stroke-width="2"
                    stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round" />
        ...
        <div class="flex flex-row items-center gap-x-3">
          <a id="deleteTask-${task.id}" href="#" class="my-auto font-semibold text-taskia-red border border-taskia-red p-[12px_20px] h-12 rounded-full">Delete</a>
        ${
          task.isCompleted === false
            ? `<a
              href="#"
